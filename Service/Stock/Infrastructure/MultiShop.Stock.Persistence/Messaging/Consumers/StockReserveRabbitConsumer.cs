...
{
    public class StockReserveRabbitConsumer : BackgroundService
    {
        ...
        public StockReserveRabbitConsumer(
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (sender, ea) =>
            {
                ...
                var @event = JsonSerializer.Deserialize<StockReserveRequestedEvent>(message);

                if (@event != null)
                {
                    using var scope = _serviceProvider.CreateScope();
                    var handler = scope.ServiceProvider
                        .GetRequiredService<StockReserveRequestedConsumer>();
