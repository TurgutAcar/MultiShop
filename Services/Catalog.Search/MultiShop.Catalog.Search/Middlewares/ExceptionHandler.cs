...
using MultiShop.Catalog.Search.Exceptions;
        ...
        public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
        {
            ...
            switch (exception)
            {
                case ValidationException validationEx:
                    ...
                    break;
                case InvalidOperationException invalidOpEx when invalidOpEx.Message.Contains("AuthorizationPolicy"):
                    httpContext.Response.StatusCode = StatusCodes.Status403Forbidden;
                    criticality = UiCriticality.Medium;
                    errorResult = Result<string>.Failure(403, invalidOpEx.Message);
                    errorResult.Criticality = criticality;

                    _logger.LogWarning(
                        invalidOpEx,
                        "Authorization policy bulunamadı. Path: {Path}",
                        httpContext.Request.Path
                    );
                    break;
